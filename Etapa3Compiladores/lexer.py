...
class Token:
    def __init__(self, tipo, valor, linha, coluna):
        # Tipo do token (identificador, palavra reservada, operador, etc.)
        self.tipo = tipo
...
def criar_token(estado, lexema, linha, coluna):
    ...
    # Se o estado não for reconhecido, assume 'Desconhecido'
    tokenDoEstado = tipos_tokens.get(estado, 'Desconhecido')
...
def abrir_arquivo(nome_arquivo):
    try:
        ...
        # Sai do programa com código de erro 1 em caso de arquivo não encontrado
        sys.exit(1)
    except Exception as e:  # Captura qualquer outra exceção não prevista
        # Imprime uma mensagem de erro genérica
        print(f"Erro ao abrir o arquivo: {e}")
        # Sai do programa com código de erro 1 em caso de erro não previsto
        sys.exit(1)
...
def proximo_token_e_erros(linhasArq, lista_tokens, lista_erros):
    ...
    for linha in linhasArq:
        ...
        while c < len(linha):
            ...
            if caracter == '<' and linha[c:c+2] == '<<':
                ignore_mode = True
                ignore_start_line = l
                ignore_start_col = c  # Contabiliza o primeiro '<'
            ...
            if estado in estados_aceitacao:
                if estado == 'q39':
                    ...
                    if estado == 'q51':
                        ...
                        if lexemaReserved != "TK_KW_INVALIDA":
                            lista_tokens.append(criar_token(
                                estado, lexema, l, c - len(lexema)))
                            lexema = ''
                            estado = 'q0'
                            c -= 1
                            continue
                ...
                lista_tokens.append(criar_token(
                    estado, lexema, l, (c+1)-len(lexema)))
                lexema = ''
                estado = 'q0'
                continue
            ...
            # Verifica se o estado atual é um estado de erro
                ...
                erro = criar_erro(msg, l, c)
                lista_erros.append(erro)
                # Não retorna token se comentário for aberto com mal formatação
                return None
    ...
    if ignore_mode:
        ...
        erro = criar_erro(msg, ignore_start_line, ignore_start_col)
        lista_erros.append(erro)
...
def printaTokens(listaTokens):
    ...
    tipos_especiais = ['TK_INT', 'TK_FLOAT', 'TK_DATA', 'TK_CADEIA', 'TK_ENDERECO', 'TK_ID', 'TK_ROTINA',
                       'TK_FIM_ROTINA', 'TK_SE', 'TK_SENAO', 'TK_IMPRIMA', 'TK_LEIA', 'TK_PARA', 'TK_ENQUANTO']
    ...
    for token in listaTokens:
        ...
        if token.linha == linha_anterior:
            ...
            # Se não, imprime o número da linha centralizado
            linha_str = str(token.linha).center(5)
        ...
        print(f"|{linha_str}|{str(token.coluna).center(5)}|{
              token.tipo.center(15)}|{lexema_str}|")
        print("+-----+-----+---------------+---------------------------+")
        ...
        if erro_presente:
            for erro in lista_erros:
                if erro.linha == num:
                    ...
                    print(
                        f"{' ' * (quant_digitos+2)}erro linha {erro.linha} coluna {erro.coluna}: {erro.msg}")
...
def printaQtdUsosToken(listaTokens):
    ...
    for token in listaTokens:
        if token.tipo in contagem:
            # Incrementa o contador se o tipo de token já existe no dicionário
            contagem[token.tipo] += 1
        else:
            # Inicia o contador em 1 se o tipo de token é encontrado pela primeira vez
            contagem[token.tipo] = 1
...
analisador_lexico("codigo_fonte3.cic")
